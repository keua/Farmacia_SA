                                    ...
                                    if (err) {
                                        ...
                                        drugstore.getMedicines({
                                            ...
                                            function (err, medicineToUpdate) {
                                                ...
                                                req.db.driver.execQuery(
                                                    "UPDATE drugstore_medicines set quantity = ?, PriceUnit=? WHERE medicines_id=? AND drugstore_id=?", [difference, params.PriceUnit, medicineFromOrder[0].id, medicineFromOrder[0].drugstoreId],
                                                    function (err, data) {
                                                        if (err)
                                                            console.log(err);
                                                    })
                                                ...
                                                req.db.driver.execQuery(
                                                    "UPDATE order_medicines set quantity = ?, PriceUnit=? WHERE medicines_id=? AND drugstoreId=?", [params.quantity, params.PriceUnit, medicineFromOrder[0].id, medicineFromOrder[0].drugstoreId],
                                                    function (err, data) {
                                                        if (err)
                                                            console.log(err);
                                                    });
                ...
                if (err) {
                    ...
                    req.models.order.get(req.params.order_id, function (err, order) {
                        if (err) {
                            ...
                            order.getMedicines({
                                    medicines_id: req.params.medicine_id
                                },
                                function (err, medicineFromOrder) {
                                    if (err) {
                                        if (err.code == orm.ErrorCodes.NOT_FOUND)
                                            ...
                                            if (err) {
                                                ...
                                                drugstore.getMedicines({
                                                    ...
                                                    function (err, medicineToUpdate) {
                                                        ...
                                                        req.db.driver.execQuery(
                                                            "UPDATE drugstore_medicines set quantity = ? WHERE medicines_id=? AND drugstore_id=?", [newQuantity, medicineFromOrder[0].id, medicineFromOrder[0].drugstoreId],
                                                            function (err, data) {
                                                                if (err)
                                                                    console.log(err);
                                                            });
